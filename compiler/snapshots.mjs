...
export function captureSnapshots(root, screens) {
  const screen = screens[0]?.ir?.screen ?? 'Screen';
  const outDir = path.join(root, '.aic/snapshots');
  fs.mkdirSync(outDir, { recursive: true });

  const iosFile = path.join(outDir, `${screen}.ios.png`);
  const androidFile = path.join(outDir, `${screen}.android.png`);
