...
function semanticCheck(ir, tokens) {
  const errors = [];
...
function validate() {
  ...
  for (const { file, ir } of screens) {
    ...
    if (!validateSchema(ir)) {
      ...
      problems.push(...semanticCheck(ir, tokens));
    }
    if (problems.length) {
      failed = true;
      console.error(`✗ ${file}`);
...
if (cmd === 'validate') validate();
  ...
  console.log('usage: node compiler/aic.mjs <validate|build|doctor:ios|dry-run:ios|dev:ios|doctor:android|dry-run:android|dev:android|snapshots>');
  process.exit(1);
}
